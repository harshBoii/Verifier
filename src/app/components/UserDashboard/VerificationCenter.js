...
const VerificationDashboard = ({ userId }) => {
  const [isBankModalOpen, setBankModalOpen] = useState(false);
  const [isPfModalOpen, setPfModalOpen] = useState(false);
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        try {
          ...
          if (res.ok) {
            setVerifiedLocation({ latitude, longitude });
            // After geotagging, move to DigiLocker step
            await initiateDigiLockerVerification('address');
          } else {
            alert('❌ Failed to verify geolocation.');
          }
        } catch (err) {
          console.error(err);
          alert('❌ Error sending location.');
  ...
  // 🔹 Step 2: DigiLocker Initiation
  const initiateDigiLockerVerification = async (section) => {
    try {
      ...
      if (data.url) {
        ...
        alert('❌ Failed to initiate DigiLocker verification.');
      }
    } catch (err) {
      console.error(err);
      alert('❌ Error initiating DigiLocker verification.');
    }
  };
  ...
  const verificationRows = [
    {
      ...
      description:
        'Verify your current address using both live location and Aadhaar (DigiLocker).',
      status: verificationStatus.address,
      action: handleGeoTagging, // First geotagging → then DigiLocker
    },
    {
      type: 'bank',
      icon: <FiDollarSign className="text-green-500" size={24} />,
      title: 'Bank Account Verification',
      description: 'Verify your bank account using secure penny drop verification.',
      status: verificationStatus.bank,
      action: () => setBankModalOpen(true),
    },
    {
      type: 'pf',
      icon: <FiShield className="text-purple-500" size={24} />,
      title: 'PF / UAN Verification',
      description: 'Verify your PF account directly from EPFO via DigiLocker.',
      status: verificationStatus.pf,
      action: () => initiateDigiLockerVerification('pf'),
    },
    {
      type: 'education',
      icon: <FiBookOpen className="text-orange-500" size={24} />,
      title: 'Education Verification',
      description: 'Verify your academic records via DigiLocker (NAD/university).',
      status: verificationStatus.education,
      action: () => initiateDigiLockerVerification('education'),
    },
  ];
