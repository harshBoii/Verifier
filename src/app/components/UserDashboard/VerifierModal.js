...
const VerifierModal = ({ isOpen, onClose, onVerifierSelect }) => {
    const [mode, setMode] = useState('direct'); // 'direct' or 'find'
                    ...
                    {mode === 'direct' && (
                        <div className="space-y-4">
                            ...
                            <button 
                                ...
                                onClick={() => handleSelect(directEmail)}
                            >
                                <FiMail /> Use this Email
                            </button>
                        </div>
                    )}
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex gap-2">
                                <input 
                                    ...
                                    className="flex-grow px-4 py-3 bg-gray-100 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 transition"
                                />
                                <button 
                                    onClick={handleFindSeniors} 
                                    className="p-3 bg-gray-200 text-gray-600 rounded-lg hover:bg-gray-300 transition" 
                                    disabled={loading}
                                >
                            ...
                            {seniors.length > 0 && (
                                <ul className="mt-4 border-t border-gray-200 divide-y divide-gray-200">
                                    {seniors.map(senior => (
                                        <li 
                                            key={senior.id} 
                                            onClick={() => handleSelect(senior.email)}
